...
当时 node 使用 npm 管理依赖包，这很方便，但是也有很多问题，依赖太复杂，代码跟踪复杂等，
在设计框架之初，我们一直在想能不能换一种组织代码的思路，而不是通过一个个零碎的依赖包来构建我们的应用。
基于此 candyjs 采用了一套基于别名的类的加载模式，架构上尽量减少了 npm 包的管理。

至 2018 年函数式编程很火，但 candyjs 并没有采用这种编程方式，而是使用了面向对象的技术，原因如下：
